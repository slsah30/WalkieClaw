#!/usr/bin/env python3
"""
AIPI Lite -> OpenClaw VPS Voice Bridge (Multi-Device)
=====================================================
...
def _extract_tts_from_session(device_ip: str = "") -> str:
    ...
    try:
        with open(sessions_index) as f:
            idx = json.load(f)
        user_key = f"walkieclaw-{device_ip}" if device_ip else "aipi-lite-voice"
...
async def _openclaw_chat_completions(text: str, device_ip: str = "") -> str:
    ...
    user_id = f"walkieclaw-{device_ip}" if device_ip else "aipi-lite-voice"
...
async def main():
    log.info("=" * 60)
    log.info("  AIPI Lite -> OpenClaw VPS Voice Bridge (Multi-Device)")
    log.info("=" * 60)
