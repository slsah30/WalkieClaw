# AIPI Lite → OpenClaw Bridge: Tailscale Networking Guide
...
┌──────────┐  UDP/HTTP   ┌───────────────┐  Tailscale    ┌──────────────────────────┐
│ AIPI Lite│ ──────────> │ Subnet Router │ ═══════════>  │ VPS (100.x.x.x)         │
...
└──────────┘                                             └──────────────────────────┘
  ...
  - VPS runs bridge.py + Blam on the same box
...
┌──────────┐  UDP/HTTP   ┌──────────────────┐  Tailscale   ┌──────────────────┐
│ AIPI Lite│ ──────────> │ Local Machine    │ ═══════════> │ VPS (100.x.x.x) │
│ ESP32-S3 │ <────────── │ (Tailscale node) │ <══════════  │  └─ OpenClaw/Blam│
│          │             │  ├─ bridge.py    │              └──────────────────┘
...
└──────────┘             │  └─ TTS engine   │
  ...
  - Only the text query goes over Tailscale to Blam on VPS
...
Everything runs on your VPS. The AIPI Lite sends audio over your subnet
router → Tailscale → VPS. Simplest setup, but audio traverses the WAN.

### Network Requirements

- Your subnet router advertises the AIPI Lite's LAN (e.g., `192.168.1.0/24`)
...
In `aipi-openclaw.yaml`, set the bridge host to your **VPS Tailscale IP**:

```yaml
substitutions:
  device_name: "aipi-openclaw"
  friendly_name: "AIPI OpenClaw Terminal"
  # Your VPS Tailscale IP (find with: tailscale ip -4 on VPS)
...
# --- ESPHome Device ---
# AIPI Lite's LAN IP — reachable from VPS via subnet routing
ESPHOME_HOST=192.168.1.50
ESPHOME_PORT=6053

# --- OpenClaw ---
# Blam is on the SAME machine, so use localhost
...
From your VPS, confirm you can reach the AIPI Lite's LAN IP:
...
→ Whisper STT → Blam → TTS → VPS HTTP → Tailscale → subnet router → ESP32
...
a mini PC, etc). Only text goes over Tailscale to Blam. Audio stays local.

### Network Flow

1. ESP32 sends UDP audio to local bridge (LAN, <1ms)
2. Bridge transcribes locally with Whisper (no network)
3. Bridge sends text to Blam over Tailscale (small payload, fast)
...
substitutions:
  device_name: "aipi-openclaw"
  friendly_name: "AIPI OpenClaw Terminal"
  # Your LOCAL bridge machine's LAN IP
...
# --- ESPHome Device ---
# AIPI Lite LAN IP (same subnet, direct)
ESPHOME_HOST=192.168.1.50
ESPHOME_PORT=6053

# --- OpenClaw ---
# Blam on VPS, reached via Tailscale
...
- Blam query: Tailscale tunnel (~50-200ms for text)
...
Since you're already running Blam on your VPS and you've been optimizing
token costs, **start with Option A** to validate the full pipeline works
end-to-end. Then migrate to **Option B** if latency bothers you — just
move bridge.py to a local machine and change the .env file. The ESPHome
firmware and bridge code are identical either way; only the IPs change.

---

## Testing the Tailscale Path

### From VPS → AIPI Lite (Option A)
...
### From AIPI Lite → VPS (verify ESP32 can reach VPS)
